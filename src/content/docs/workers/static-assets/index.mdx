...
export default {
  async fetch(request) {
    // URL da sua API no HostGator (substitua pela sua)
    const API_URL = 'https://api.syncnf.com.br';

    // Modifica a requisição para apontar para sua API
    const newRequest = new Request(API_URL, request);

    // Adiciona headers para evitar bloqueio do ModSecurity
    newRequest.headers.set('Accept', 'application/json');
    newRequest.headers.set('Content-Type', 'application/json');
    newRequest.headers.set('User-Agent', 'Cloudflare-Worker');
    ...
    try {
      ...
      return new Response('Erro ao acessar a API: ' + error.message, { status: 500 });
    }
  }
};
```
